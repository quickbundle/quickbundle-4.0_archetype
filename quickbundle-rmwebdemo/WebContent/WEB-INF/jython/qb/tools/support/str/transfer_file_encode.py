            ...
            try:
                ...
                if s_encode == 'GBK' or s_encode == 'GB2312':
                    if temp_file.endswith('xml') or temp_file.endswith('xsl'):
                        if re.search('''<\?xml[^>]+?encoding="((gb2312)|(GBK))"[^>]*?\?>''', s2, re.IGNORECASE):
                            p_encode = re.compile('''encoding="((gb2312)|(GBK))"''', re.IGNORECASE)
                            s2 = p_encode.sub('encoding="' + d_encode + '"', s2, 1)
                            print 'info: subing ' + str(p_encode.findall(s2_original, re.IGNORECASE)) + '-->' + 'encoding="' + d_encode + '"'
                            sub_count = sub_count + 1
                    elif temp_file.endswith('jsp') or temp_file.endswith('htm') or temp_file.endswith('html') or temp_file.endswith('shtml'):
                        if re.search('charset=((gb2312)|(GBK))', s2, re.IGNORECASE):
                            p_charset = re.compile('charset=((gb2312)|(GBK))', re.IGNORECASE)
                            s2 = p_charset.sub('charset=' + d_encode, s2, 2)
                            print 'info: subing ' + str(p_charset.findall(s2_original, re.IGNORECASE)) + '-->' + 'charset=' + d_encode
                            sub_count = sub_count + 1
                        if re.search('pageEncoding="((gb2312)|(GBK))"', s2, re.IGNORECASE):
                            p_pageEncoding = re.compile('pageEncoding="((gb2312)|(GBK))"', re.IGNORECASE)
                            s2 = p_pageEncoding.sub('pageEncoding="' + d_encode + '"', s2, 2)
                            print 'info: subing ' + str(p_pageEncoding.findall(s2_original, re.IGNORECASE)) + '-->' + 'pageEncoding=' + d_encode
