...
<%  //判断是否只读
	...
	if("1".equals(request.getParameter(IRmMessageConstants.REQUEST_IS_READ_ONLY)) || "1".equals(request.getAttribute(IRmMessageConstants.REQUEST_IS_READ_ONLY))) {
		isReadOnly = true;
	}
%>
<%  //取出List
...
<%  //是否跳往打印页面
	if("1".equals(request.getParameter("isExport"))) {  //如果isExport参数等于1
		...
		response.sendRedirect(request.getContextPath() + "/modules/message/rmmessage/exportRmMessage_custom.jsp");  //跳转到定制导出页面
		return;
	}
%>
...
<script type="text/javascript">
	function findCheckbox_onClick() {  //从多选框到修改页面
		...
		window.location="<%=request.getContextPath()%>/rmmessage/update/" + ids;
	}
	function deleteMulti_onClick(){  //从多选框物理删除多条记录
    	...
    	form.action="<%=request.getContextPath()%>/rmmessage/delete?ids=" + ids;
    	form.submit();
	}
	function list_onClick(){  //简单的模糊查询
    	form.action="<%=request.getContextPath()%>/rmmessage";
    	form.submit();
  	}
  	
	function toImport_onClick() {  //到导入页面
		window.location="<%=request.getContextPath()%>/rmmessage/import";
	}
	...
	function toAdd_onClick() {  //到增加记录页面
		window.location="<%=request.getContextPath()%>/rmmessage/insert";
	}
	function refresh_onClick() {  //刷新本页
		form.submit();
	}
	function detail_onClick(thisId) {  //实现转到详细页面
		form.action="<%=request.getContextPath()%>/rmmessage/detail/" + thisId;
		form.submit();
	}
...
<%=isReadOnly ? "<input type=\"hidden\" name=\"" + IRmMessageConstants.REQUEST_IS_READ_ONLY + "\" value=\"1\">" : ""%></form>

<%--begin 生成页面汇总，正式部署前删除以下代码 --%>
<div id="div_funcNode" style="padding:20px 10px 10px 0px; display:none" align="right">
	<a class="aul" target="_blank" href="<%=request.getContextPath()%>/rmmessage/ajax">Ajax</a>
	<a class="aul" target="_blank" href="<%=request.getContextPath()%>/rmmessage/statistic/chart">图表统计</a>
	<a class="aul" target="_blank" href="<%=request.getContextPath()%>/rmmessage/statistic/flash">Flash图表</a>
	<a class="aul" target="_blank" href="<%=request.getContextPath()%>/rmmessage/statistic/table">交叉统计</a>
	<a class="aul" target="_blank" href="<%=request.getContextPath()%>/rmmessage?<%=IRmMessageConstants.REQUEST_IS_READ_ONLY%>=1">只读模式</a>
	</div>
<%--end --%>
