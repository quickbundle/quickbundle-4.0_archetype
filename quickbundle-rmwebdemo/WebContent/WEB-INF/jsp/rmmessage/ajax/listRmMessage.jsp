...
Ext.onReady(function(){
			...
			items:[
				...
				},
				{
					...
					items: [{
						...
						urlPath: '<%=request.getContextPath()%>/api/rmmessage/reference?referenceInputType=radio'
					}]
				}
            ]
        },
    ...
    var columnModel = new Ext.ux.grid.LockingColumnModel({
        ...
        columns: [
			...
			},
			{
				...
				editor: new fm.ReferenceField({ urlPath: '<%=request.getContextPath()%>/api/rmmessage/reference?referenceInputType=radio' })
			},
			{
	...
	var store = new Ext.data.JsonStore({
	    ...
	    proxy: new Ext.data.HttpProxy({
	        url: '<%=request.getContextPath()%>/api/rmmessage',
	    	method: 'GET'
	    }),
        remoteSort: true,
...
<%}%>
           			Ext.Ajax.request({
	           			url: '<%=request.getContextPath()%>/api/rmmessage/batch',
	           			method: 'POST',
	           			params: params,
				...
				if (selModel.hasSelection()) {
					Ext.Msg.confirm("警告", "确定要删除吗？", function(button) {
						if (button == "yes") {
							...
							if(ids != '') {
			           			Ext.Ajax.request({
				           			url: '<%=request.getContextPath()%>/api/rmmessage/delete', 
				           			method: 'POST',
				           			params: {ids: ids},
				           			success: function(response) {
