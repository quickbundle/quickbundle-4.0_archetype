    ...
    protected Connection getConnection() throws SQLException {
        ...
        if(RmConfig.systemDebugMode()) {
        	...
        	if(callStack.length() > 10000) {
        		...
        		sb.append("[stack string length:" + callStack.length() + "]");
        	}
        	callStack = callStack.replaceAll(", ", ",<br/>\n");
        	sb.append("<div nowrap>");
        	sb.append("<input type=\"button\" value=\"call stack\" onclick=\"if(this.parentNode.children[1].style.display=='block'){this.parentNode.children[1].style.display='none'}else{this.parentNode.children[1].style.display='block'}\">");
        	sb.append("<span style=\"display:none\">");
        	sb.append(callStack);
        	sb.append("</span>");
        	sb.append("</div>");
        }
        String requester = sb.toString();
