//代码生成时,文件路径: e:/download/QbRmWebDemo/src/main/java/orgauth/rmpartyrole/service/impl/RmPartyRoleService.java
 ...
 * 
 * 文件名称: org.quickbundle.orgauth.rmpartyrole.service.impl --> RmPartyRoleService.java
 * 
 * 功能描述:
 * 
...
package org.quickbundle.orgauth.rmpartyrole.service.impl;
...
public class RmPartyRoleService extends RmService implements IRmPartyRoleService, IRmPartyRoleConstants {
    
    /**
     * dao 表示: 数据访问层的实例
     */
    private IRmPartyRoleDao dao = null;
     ...
     * @param dao
     */
    public void setDao(IRmPartyRoleDao dao) {
        this.dao = dao;
    }
     ...
     * @return 返回新生成的id数组
     */
    public String[] insert(RmPartyRoleVo[] vos) {
    ...
    public RmPartyRoleVo find(String id) {
		RmPartyRoleVo vo = getDao().find(id);
     ...
     * @return 成功更新的记录最终数量
     */
	public int update(RmPartyRoleVo[] vos) {
	 ...
	 */
	public int[] insertUpdateBatch(RmPartyRoleVo[] vos) {
		int[] sum_insert_update = new int[2];
		List<RmPartyRoleVo> lInsert = new ArrayList<RmPartyRoleVo>();
		List<RmPartyRoleVo> lUpdate = new ArrayList<RmPartyRoleVo>();
		...
		if(lInsert.size() > 0) {
			sum_insert_update[0] = insert(lInsert.toArray(new RmPartyRoleVo[0])).length;
		}
		if(lUpdate.size() > 0) {
			sum_insert_update[1] = update(lUpdate.toArray(new RmPartyRoleVo[0]));
		}
     ...
     * @return 查询到的VO列表
     */
    public List<RmPartyRoleVo> queryByCondition(String queryCondition, String orderStr) {
     ...
     * @return 查询到的VO列表
     */
    public List<RmPartyRoleVo> queryByCondition(String queryCondition, String orderStr, int startIndex, int size) {
    ...
    public List<RmPartyRoleVo> queryByCondition(String queryCondition, String orderStr, int startIndex, int size, boolean selectAllClumn) {
        List<RmPartyRoleVo> lResult = getDao().queryByCondition(queryCondition, orderStr, startIndex, size, selectAllClumn);
