//代码生成时,文件路径: E:/QbRmWebDemo/src/main/java/orgauth/rmpartyview/service/impl/RmPartyViewService.java
//代码生成时,系统时间: 2010-11-28 17:40:26
//代码生成时,操作系统用户: Administrator

/*
 * 系统名称:单表模板 --> QbRmWebDemo
 * 
 * 文件名称: org.quickbundle.orgauth.rmpartyview.service.impl --> RmPartyViewService.java
 * 
 * 功能描述:
 * 
 * 版本历史: 2010-11-28 17:40:26 创建1.0.0版 (白小勇)
 *  
 */

package org.quickbundle.orgauth.rmpartyview.service.impl;
...
public class RmPartyViewService extends RmService implements IRmPartyViewService, IRmPartyViewConstants {
    
    /**
     * dao 表示: 数据访问层的实例
     */
    private IRmPartyViewDao dao = null;
     ...
     * @param dao
     */
    public void setDao(IRmPartyViewDao dao) {
        this.dao = dao;
    }
     ...
     * @return 返回新生成的id数组
     */
    public String[] insert(RmPartyViewVo[] vos) {
     ...
     * @return 查询到的VO对象
     */
    public RmPartyViewVo find(String id) {
		RmPartyViewVo vo = getDao().find(id);
     ...
     * @return 成功更新的记录数
     */
    public int update(RmPartyViewVo vo) {
	 ...
	 */
	public int[] insertUpdateBatch(RmPartyViewVo[] vos) {
		int[] sum_insert_update = new int[2];
		List<RmPartyViewVo> lInsert = new ArrayList<RmPartyViewVo>();
		List<RmPartyViewVo> lUpdate = new ArrayList<RmPartyViewVo>();
		...
		if(lInsert.size() > 0) {
			sum_insert_update[0] = insert(lInsert.toArray(new RmPartyViewVo[0])).length;
		}
		if(lUpdate.size() > 0) {
			sum_insert_update[1] = update(lUpdate.toArray(new RmPartyViewVo[0]));
		}
     ...
     * @return 查询到的VO列表
     */
    public List<RmPartyViewVo> queryByCondition(String queryCondition, String orderStr) {
     ...
     * @return 查询到的VO列表
     */
    public List<RmPartyViewVo> queryByCondition(String queryCondition, String orderStr, int startIndex, int size) {
     ...
     * @return 查询到的VO列表
     */
    public List<RmPartyViewVo> queryByCondition(String queryCondition, String orderStr, int startIndex, int size, boolean selectAllClumn) {
        List<RmPartyViewVo> lResult = getDao().queryByCondition(queryCondition, orderStr, startIndex, size, selectAllClumn);
