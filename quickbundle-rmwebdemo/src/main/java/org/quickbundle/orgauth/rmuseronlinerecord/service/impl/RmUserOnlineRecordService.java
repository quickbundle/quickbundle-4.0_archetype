//代码生成时,文件路径: e:/download/QbRmWebDemo/src/main/java/orgauth/rmuseronlinerecord/service/impl/RmUserOnlineRecordService.java
 ...
 * 
 * 文件名称: org.quickbundle.orgauth.rmuseronlinerecord.service.impl --> RmUserOnlineRecordService.java
 * 
 * 功能描述:
 * 
...
package org.quickbundle.orgauth.rmuseronlinerecord.service.impl;
...
public class RmUserOnlineRecordService extends RmService implements IRmUserOnlineRecordService, IRmUserOnlineRecordConstants {
     ...
     * dao 表示: 数据访问层的实例
     */
    private IRmUserOnlineRecordDao dao = null;
     ...
     * @return
     */
    public IRmUserOnlineRecordDao getDao() {
        return dao;
    }
     ...
     * @param dao
     */
    public void setDao(IRmUserOnlineRecordDao dao) {
        this.dao = dao;
    }
     ...
     * @return 返回新生成的id数组
     */
    public String[] insert(RmUserOnlineRecordVo[] vos) {
    ...
    public RmUserOnlineRecordVo find(String id) {
		RmUserOnlineRecordVo vo = getDao().find(id);
     ...
     * @return 成功更新的记录最终数量
     */
	public int update(RmUserOnlineRecordVo[] vos) {
	 ...
	 */
	public int[] insertUpdateBatch(RmUserOnlineRecordVo[] vos) {
		int[] sum_insert_update = new int[2];
		List<RmUserOnlineRecordVo> lInsert = new ArrayList<RmUserOnlineRecordVo>();
		List<RmUserOnlineRecordVo> lUpdate = new ArrayList<RmUserOnlineRecordVo>();
		...
		if(lInsert.size() > 0) {
			sum_insert_update[0] = insert(lInsert.toArray(new RmUserOnlineRecordVo[0])).length;
		}
		if(lUpdate.size() > 0) {
			sum_insert_update[1] = update(lUpdate.toArray(new RmUserOnlineRecordVo[0]));
		}
     ...
     * @return 查询到的VO列表
     */
    public List<RmUserOnlineRecordVo> queryByCondition(String queryCondition, String orderStr) {
     ...
     * @return 查询到的VO列表
     */
    public List<RmUserOnlineRecordVo> queryByCondition(String queryCondition, String orderStr, int startIndex, int size) {
    ...
    public List<RmUserOnlineRecordVo> queryByCondition(String queryCondition, String orderStr, int startIndex, int size, boolean selectAllClumn) {
        List<RmUserOnlineRecordVo> lResult = getDao().queryByCondition(queryCondition, orderStr, startIndex, size, selectAllClumn);
