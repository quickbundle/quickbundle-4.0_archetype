//代码生成时,文件路径: E:/QbRmWebDemo/src/main/java/orgauth/rmpartytyperelationrule/service/impl/RmPartyTypeRelationRuleService.java
 ...
 * 
 * 文件名称: org.quickbundle.orgauth.rmpartytyperelationrule.service.impl --> RmPartyTypeRelationRuleService.java
 * 
 * 功能描述:
 * 
...
package org.quickbundle.orgauth.rmpartytyperelationrule.service.impl;
...
public class RmPartyTypeRelationRuleService extends RmService implements IRmPartyTypeRelationRuleService, IRmPartyTypeRelationRuleConstants {
    
    /**
     * dao 表示: 数据访问层的实例
     */
    private IRmPartyTypeRelationRuleDao dao = null;
     ...
     * @return
     */
    public IRmPartyTypeRelationRuleDao getDao() {
        return dao;
    }
     ...
     * @param dao
     */
    public void setDao(IRmPartyTypeRelationRuleDao dao) {
        this.dao = dao;
    }
     ...
     * @return 返回新生成的id数组
     */
    public String[] insert(RmPartyTypeRelationRuleVo[] vos) {
    ...
    public RmPartyTypeRelationRuleVo find(String id) {
		RmPartyTypeRelationRuleVo vo = getDao().find(id);
	 ...
	 */
	public int[] insertUpdateBatch(RmPartyTypeRelationRuleVo[] vos) {
		int[] sum_insert_update = new int[2];
		List<RmPartyTypeRelationRuleVo> lInsert = new ArrayList<RmPartyTypeRelationRuleVo>();
		List<RmPartyTypeRelationRuleVo> lUpdate = new ArrayList<RmPartyTypeRelationRuleVo>();
		...
		if(lInsert.size() > 0) {
			sum_insert_update[0] = insert(lInsert.toArray(new RmPartyTypeRelationRuleVo[0])).length;
		}
		if(lUpdate.size() > 0) {
			sum_insert_update[1] = update(lUpdate.toArray(new RmPartyTypeRelationRuleVo[0]));
		}
     ...
     * @return 查询到的VO列表
     */
    public List<RmPartyTypeRelationRuleVo> queryByCondition(String queryCondition, String orderStr) {
     ...
     * @return 查询到的VO列表
     */
    public List<RmPartyTypeRelationRuleVo> queryByCondition(String queryCondition, String orderStr, int startIndex, int size) {
    ...
    public List<RmPartyTypeRelationRuleVo> queryByCondition(String queryCondition, String orderStr, int startIndex, int size, boolean selectAllClumn) {
        List<RmPartyTypeRelationRuleVo> lResult = getDao().queryByCondition(queryCondition, orderStr, startIndex, size, selectAllClumn);
