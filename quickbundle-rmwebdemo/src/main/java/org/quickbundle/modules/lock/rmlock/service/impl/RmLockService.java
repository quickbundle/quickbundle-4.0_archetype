...
public class RmLockService extends RmService implements IRmLockService, IRmLockConstants {
    ...
    public int deleteLock(String user_id, String value, String bs_keyword) {
		return RmProjectHelper.getCommonServiceInstance().doUpdate("delete from RM_LOCK where USER_ID=? and BS_KEYWORD=? and LOCK_CONTENT=?", new String[]{user_id, bs_keyword, value});
    }
		...
		try {
			...
			if(e.getMessage().indexOf("IDXU_LOCK_BK_CONTENT") != -1) {
				if(vo.getUser_id() != null && vo.getUser_id().length() > 0) {
					...
					int sum = RmProjectHelper.getCommonServiceInstance().doQueryForInt("select count(ID) from RM_LOCK where USER_ID=? and BS_KEYWORD=? and LOCK_CONTENT=?", new String[]{vo.getUser_id(), vo.getBs_keyword(), vo.getLock_content()});
					if(sum > 0) {
						return true;
					}
				}
