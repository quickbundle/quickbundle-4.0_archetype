//代码生成时,文件路径: E:/QbRmWebDemo/src/main/java/modules/log/rmlogtype/service/impl/RmLogTypeService.java
 ...
 * 
 * 文件名称: org.quickbundle.modules.log.rmlogtype.service.impl --> RmLogTypeService.java
 * 
 * 功能描述:
 * 
...
package org.quickbundle.modules.log.rmlogtype.service.impl;
...
public class RmLogTypeService extends RmService implements IRmLogTypeService, IRmLogTypeConstants {
    
    /**
     * dao 表示: 数据访问层的实例
     */
    private IRmLogTypeDao dao = null;
     ...
     * @param dao
     */
    public void setDao(IRmLogTypeDao dao) {
        this.dao = dao;
    }
     ...
     * @return 返回新生成的id数组
     */
    public String[] insert(RmLogTypeVo[] vos) {
	 ...
	 */
	public int[] insertUpdateBatch(RmLogTypeVo[] vos) {
		int[] sum_insert_update = new int[2];
		List<RmLogTypeVo> lInsert = new ArrayList<RmLogTypeVo>();
		List<RmLogTypeVo> lUpdate = new ArrayList<RmLogTypeVo>();
		...
		if(lInsert.size() > 0) {
			sum_insert_update[0] = insert(lInsert.toArray(new RmLogTypeVo[0])).length;
		}
		if(lUpdate.size() > 0) {
			sum_insert_update[1] = update(lUpdate.toArray(new RmLogTypeVo[0]));
		}
     ...
     * @return 查询到的VO列表
     */
    public List<RmLogTypeVo> queryByCondition(String queryCondition, String orderStr) {
     ...
     * @return 查询到的VO列表
     */
    public List<RmLogTypeVo> queryByCondition(String queryCondition, String orderStr, int startIndex, int size) {
    ...
    public List<RmLogTypeVo> queryByCondition(String queryCondition, String orderStr, int startIndex, int size, boolean selectAllClumn) {
        List<RmLogTypeVo> lResult = getDao().queryByCondition(queryCondition, orderStr, startIndex, size, selectAllClumn);
