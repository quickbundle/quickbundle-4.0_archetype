//代码生成时,文件路径: E:/platform/myProject/svn/oss/quickbundle/trunk/QbRmWebDemo/src/main/java/org/quickbundle/third/quartz/rmschedulerevent/service/impl/RmSchedulerEventService.java
 ...
 * 
 * 文件名称: org.quickbundle.third.quartz.rmschedulerevent.service.impl --> RmSchedulerEventService.java
 * 
 * 功能描述:
 * 
...
package org.quickbundle.third.quartz.rmschedulerevent.service.impl;
...
public class RmSchedulerEventService extends RmService implements IRmSchedulerEventService, IRmSchedulerEventConstants {
    
    /**
     * dao 表示: 数据访问层的实例
     */
    private IRmSchedulerEventDao dao = null;
     ...
     * @return
     */
    public IRmSchedulerEventDao getDao() {
        return dao;
    }
     ...
     * @param dao
     */
    public void setDao(IRmSchedulerEventDao dao) {
        this.dao = dao;
    }
     ...
     * @return 返回新生成的id数组
     */
    public String[] insert(RmSchedulerEventVo[] vos) {
    ...
    public RmSchedulerEventVo find(String id) {
		RmSchedulerEventVo vo = getDao().find(id);
     ...
     * @return 成功更新的记录最终数量
     */
	public int update(RmSchedulerEventVo[] vos) {
	 ...
	 */
	public int[] insertUpdateBatch(RmSchedulerEventVo[] vos) {
		int[] sum_insert_update = new int[2];
		List<RmSchedulerEventVo> lInsert = new ArrayList<RmSchedulerEventVo>();
		List<RmSchedulerEventVo> lUpdate = new ArrayList<RmSchedulerEventVo>();
		...
		if(lInsert.size() > 0) {
			sum_insert_update[0] = insert(lInsert.toArray(new RmSchedulerEventVo[0])).length;
		}
		if(lUpdate.size() > 0) {
			sum_insert_update[1] = update(lUpdate.toArray(new RmSchedulerEventVo[0]));
		}
     ...
     * @return 查询到的VO列表
     */
    public List<RmSchedulerEventVo> queryByCondition(String queryCondition, String orderStr) {
     ...
     * @return 查询到的VO列表
     */
    public List<RmSchedulerEventVo> queryByCondition(String queryCondition, String orderStr, int startIndex, int size) {
    ...
    public List<RmSchedulerEventVo> queryByCondition(String queryCondition, String orderStr, int startIndex, int size, boolean selectAllClumn) {
        List<RmSchedulerEventVo> lResult = getDao().queryByCondition(queryCondition, orderStr, startIndex, size, selectAllClumn);
